 ...
 */
angular.module('igl').controller('IGDocumentListCtrl', function (TableService, $scope, $rootScope, $templateCache, Restangular, $http, $filter, $modal, $cookies, $timeout, userInfoService, ToCSvc, ContextMenuSvc, ProfileAccessSvc, ngTreetableParams, $interval, ViewSettings, StorageService, $q, Notification, DatatypeService, SegmentService, IgDocumentService, ElementUtils, AutoSaveService, DatatypeLibrarySvc, SegmentLibrarySvc, TableLibrarySvc, TableService, MastermapSvc, MessageService, FilteringSvc, blockUI, PcService) {
	$scope.loading = false;
	...
	$scope.loadIGDocuments = function () {
		...
		if (userInfoService.isAuthenticated() && !userInfoService.isPending()) {
			...
			$http.get('api/igdocuments', {
				params: {
					"type": $scope.igDocumentConfig.selectedType
				}
...
//							$scope.loadPc().then(function () {}, function () {});
	...
	$scope.initIGDocument = function () {
		$scope.loading = true;
		if ($rootScope.igdocument != null && $rootScope.igdocument != undefined) $scope.gotoSection($rootScope.igdocument.metaData, 'metaData');
		$scope.loading = false;
	};
			...
			$timeout(function () {
				try {
					SegmentService.get(segment.id).then(function (result) {
						...
						$scope.loadingSelection = false;
						try {
							if ($scope.segmentsParams) $scope.segmentsParams.refresh();
						}
						catch (e) {}
			...
			$timeout(function () {
				try {
					DatatypeService.getOne(datatype.id).then(function (result) {
						...
						$scope.loadingSelection = false;
						try {
							if ($scope.datatypesParams) $scope.datatypesParams.refresh();
						}
						catch (e) {}
						$rootScope.references = [];
						$rootScope.tmpReferences = [].concat($rootScope.references);
	...
	$scope.selectMessage = function (message) {
		...
		$timeout(function () {
			try {
				...
				$scope.loadingSelection = false;
				try {
					if ($scope.messagesParams) $scope.messagesParams.refresh();
				}
				catch (e) {}
				$rootScope.$emit("event:initEditArea");
				blockUI.stop();
...
angular.module('igl').controller('ConfirmIGDocumentCloseCtrl', function ($scope, $modalInstance, $rootScope, $http) {
	...
	$scope.discardChangesAndClose = function () {
		$scope.loading = true;
		$http.get('api/igdocuments/' + $rootScope.igdocument.id, {
			timeout: 60000
		}).then(function (response) {
	...
	$scope.ConfirmIGDocumentOpenCtrl = function () {
		...
		$http.post('api/igdocuments/save', data, {
			timeout: 60000
		}).then(function (response) {
...
angular.module('igl').controller('AddDatatypeDlgCtl', function ($scope, $rootScope, $modalInstance, hl7Version, datatypes, DatatypeLibrarySvc, DatatypeService) {
...
angular.module('igl').controller('AddMasterDatatypes', function ($scope, $rootScope, $modalInstance, datatypes, DatatypeLibrarySvc, DatatypeService) {
...
angular.module('igl').controller('AddSegmentDlgCtl', function ($scope, $rootScope, $modalInstance, hl7Version, $http, SegmentService, SegmentLibrarySvc, DatatypeService, DatatypeLibrarySvc, TableService, TableLibrarySvc, IgDocumentService) {
