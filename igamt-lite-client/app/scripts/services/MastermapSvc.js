...
.service (
  ...
  function($rootScope) {
    ...
    svc.addComponent = function(c, parent){
      ...
      svc.mastermap[c.id]["type"] = "component";
      if (svc.mastermap[c.id]["usage"] === undefined){
        svc.mastermap[c.id]["usage"] = c["usage"];
      }
      if (c.datatype !== undefined){
        if (svc.mastermap[c.datatype] === undefined){
          svc.createMMElement(c.datatype);
          svc.mastermap[c.datatype]["type"] = "component";
        }
      svc.addParentsId(c.datatype, "component", [[c.id, "component"]]);
      }
      if (c.table !== undefined){
        if (svc.mastermap[c.table] === undefined){
          svc.createMMElement(c.table);
          svc.mastermap[c.table]["type"] = "table";
        }
      svc.addParentsId(c.table, "component", [[c.id, "component"]]);
      }
...
svc.getElement = function(id, type){
  ...
  console.log("requested " + id + " " + type + " : not found");
  return undefined;
}
