...
angular.module('igl').factory('SegmentService', ['$rootScope', 'ViewSettings', 'ElementUtils', '$q', '$http', 'FilteringSvc', 'userInfoService','$filter', 'TableService', function($rootScope, ViewSettings, ElementUtils, $q, $http, FilteringSvc, userInfoService,$filter, TableService) {
    var SegmentService = {
                ...
                if (parent.fields) {
                    ...
                    for (var i = 0, len = children.length; i < len; i++) {
                        ...
                        if ($rootScope.segment) {
                            children[i].sev = _.find($rootScope.segment.singleElementValues, function (sev) {
                                return sev.location == children[i].path;
                            });
                            ...
                            if (children[i].path.split(".").length - 1 == 1) {
                                ...
                                if ($rootScope.segment) {
                                    ...
                                    if (children[i].sev) {
                                        ...
                                        children[i].sev = _.find($rootScope.datatypesMap[children[i].fieldDT].singleElementValues, function (sev) {
                                            return sev.location == fieldPath;
                                        });
                                        if (children[i].sev) {
                                ...
                                if ($rootScope.segment) {
                                    ...
                                    if (children[i].sev) {
                                        ...
                                        if (children[i].sev) {
                                            ...
                                            children[i].sev = _.find($rootScope.datatypesMap[children[i].componentDT].singleElementValues, function (sev) {
        ...
        reset: function () {
            ...
            SegmentService.initCoConstraintsTable($rootScope.segment);
        },
