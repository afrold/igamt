...
public class SerializableDatatypeLibrarySummary extends SerializableSection {
	...
	public SerializableDatatypeLibrarySummary(String id, String prefix, String position, String headerLevel,
			String title, HashMap<DatatypeLink,Datatype> datatypeLinkDatatypeMap, SCOPE datatypeLibraryScope) {
			...
			if(datatypeLink!=null){
				...
				if(datatype != null && datatype.getScope().equals(datatypeLibraryScope)){
					...
					if(datatype.getLabel()!=null) {
						datatypeLibrarySummaryItemElement.addAttribute(new Attribute("label", datatype.getLabel()));
					}
					if(datatype.getDescription() != null) {
						datatypeLibrarySummaryItemElement.addAttribute(new Attribute("description", datatype.getDescription()));
					}
					if(datatype.getHl7versions() != null && !datatype.getHl7versions().isEmpty()) {
						datatypeLibrarySummaryItemElement.addAttribute(new Attribute("compatibilityVersions", String.join(",", datatype.getHl7versions())));
					}
					if(datatype.getStatus() != null) {
						datatypeLibrarySummaryItemElement.addAttribute(new Attribute("publicationStatus", datatype.getStatus().name()));
					}
					datatypeLibrarySummaryItemElement.addAttribute(new Attribute("publicationVersion", String.valueOf(datatype.getPublicationVersion())));
					if(datatype.getPurposeAndUse() != null) {
						datatypeLibrarySummaryItemElement.addAttribute(new Attribute("purposeAndUse", datatype.getPurposeAndUse()));
					}
