<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="DatatypeMetadata">
    	<xsl:element name="table">
    		...
    		<xsl:element name="thead">
    			...
    			<xsl:element name="tr">
    				<xsl:if test="$datatypeMetadata.hl7Version = 'true'">
	    				<xsl:element name="td">
	    					<xsl:text>HL7 Version</xsl:text>
	    				</xsl:element>
    				</xsl:if>
    				<xsl:if test="$datatypeMetadata.publicationDate = 'true'">
	    				<xsl:element name="td">
	    					<xsl:text>Publication Date</xsl:text>
	    				</xsl:element>
    				</xsl:if>
    				<xsl:if test="$datatypeMetadata.publicationVersion = 'true'">
	    				<xsl:element name="td">
	    					<xsl:text>Publication Version</xsl:text>
	    				</xsl:element>
    				</xsl:if>
    				<xsl:if test="$datatypeMetadata.scope = 'true'">
	    				<xsl:element name="td">
