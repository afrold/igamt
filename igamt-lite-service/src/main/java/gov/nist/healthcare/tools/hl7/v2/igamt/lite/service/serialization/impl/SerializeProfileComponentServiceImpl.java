	...
	private SerializableSection serializeProfileComponent(ProfileComponent profileComponent, Integer position,
		...
		try {
			if (profileComponent != null) {
				...
				SerializableSection serializableSection = new SerializableSection(id, profileComponent.getName(),
						segmentPosition, sectionHeaderLevel, title);
				...
				for (SubProfileComponent subProfileComponent : profileComponent.getChildren()) {
					...
					if (subProfileComponent.getFrom().equals("message")) {
						if (subProfileComponent.getAttributes().getUsage() != null) {
							if (ExportUtil.diplayUsage(subProfileComponent.getAttributes().getUsage(),
									profileComponentItemsExport)) {
								exportItem = true;
							}
						} else if (subProfileComponent.getAttributes().getOldUsage() != null) {
							if (ExportUtil.diplayUsage(subProfileComponent.getAttributes().getOldUsage(),
									profileComponentItemsExport)) {
								exportItem = true;
							}
						} else {
					...
					if (subProfileComponent.getAttributes().getCoConstraintsTable() != null) {
						CoConstraintsTable coConstraintsTable = subProfileComponent.getAttributes()
								.getCoConstraintsTable();
						if (coConstraintsTable != null && coConstraintsTable.getRowSize() > 0) {
										...
										for (ValueSetData valueSetData : coConstraintTHENColumnData.getValueSets()) {
											...
											if (table != null) {
												...
												throw new CoConstraintSerializationException(
														new TableNotFoundException(valueSetData.getTableId()),
					...
					if (subProfileComponent.getAttributes().getDynamicMappingDefinition() != null) {
						DynamicMappingDefinition dynamicMappingDefinition = subProfileComponent.getAttributes()
								.getDynamicMappingDefinition();
									...
									try {
										if (dynamicMappingItem != null && dynamicMappingItem.getDatatypeId() != null) {
											Datatype datatype = datatypeService
													.findById(dynamicMappingItem.getDatatypeId());
											if (datatype != null) {
												dynamicMappingDatatypeMap.put(dynamicMappingItem.getDatatypeId(),
														datatype);
											} else {
										...
										throw new DynamicMappingItemSerializationException(e, dynamicMappingDefinition
												.getDynamicMappingItems().indexOf(dynamicMappingItem) + 1);
									}
								}
							} catch (Exception e) {
				...
				SerializableProfileComponent serializableProfileComponent = new SerializableProfileComponent(id,
