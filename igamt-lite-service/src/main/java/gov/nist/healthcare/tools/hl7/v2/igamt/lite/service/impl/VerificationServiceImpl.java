...
public class VerificationServiceImpl implements VerificationService {
  ...
  public InputStream verifySegment2(Profile p, Profile baseP, String id, String type) {
    ...
    try {
      ...
      // result = this.validateChangeCardinality(String.valueOf(f.getMin()), f.getMax(),
      // f.getUsage());
      // generator.writeStringField("result", result);
      // generator.writeEndObject();
      //
  ...
  public ElementVerification verifyField(Profile p, Profile baseP, String id, String type) {
    ...
    //
    // ElementVerificationResult evcRst = new ElementVerificationResult("usage",
    // f.getUsage().value(), result);
    // evc.addElementVerifications(evcRst);
    //
    // result = this.validateChangeLength(String.valueOf(f.getMinLength()), f.getMaxLength());
    // evcRst = new ElementVerificationResult("minLength", String.valueOf(f.getMinLength()),
    // result);
    // evc.addElementVerifications(evcRst);
    //
    // result = this.validateChangeLength(String.valueOf(f.getMinLength()), f.getMaxLength());
    // evcRst = new ElementVerificationResult("maxLength", String.valueOf(f.getMaxLength()),
    // result);
    // evc.addElementVerifications(evcRst);
    //
    // result = this.validateChangeCardinality(String.valueOf(f.getMin()), f.getMax(),
  ...
  public ElementVerification verifyComponent(Profile p, Profile baseP, String id, String type) {
    ...
    //
    // ElementVerificationResult evcRst = new ElementVerificationResult("usage",
    // c.getUsage().value(), result);
    // evc.addElementVerifications(evcRst);
    //
    // result = this.validateChangeLength(String.valueOf(c.getMinLength()), c.getMaxLength());
    // evcRst = new ElementVerificationResult("minLength", String.valueOf(c.getMinLength()),
    // result);
    // evc.addElementVerifications(evcRst);
    //
    // result = this.validateChangeLength(String.valueOf(c.getMinLength()), c.getMaxLength());
    // evcRst = new ElementVerificationResult("maxLength", String.valueOf(c.getMaxLength()),
    // result);
    // evc.addElementVerifications(evcRst);
    //
  ...
  public ElementVerification verifyValueSet(Profile p, Profile baseP, String id, String type) {
    ...
    // ElementVerificationResult evcRst = new ElementVerificationResult("usage", c.getCodeUsage(),
    // result);
    // evc.addElementVerifications(evcRst);
  ...
  public InputStream verifyLength2(Profile p, String id, String type, String eltName,
      String eltValue) {
    // type is either Field or Component
    // eltName is minLength or maxLength
    // String currentMinLength = "";
  ...
  private void writeOneJsonResult(JsonGenerator generator, String id, String type, String eltName,
      String eltValue, String result) throws IOException {
  ...
  private String validateChangeCardinality(String currentMin, String currentMax,
    ...
    if (message.isEmpty())
      message =
          this.allowedChangesCardinality(Integer.valueOf(currentMin), currentMax, currentUsage);
    if (message.isEmpty()) {
      return "ok";
    }
    return message;
  ...
  private List<String> validUsages(String version) throws IOException {
    ...
    String jsonRules =
        IOUtils.toString(this.getClass().getResourceAsStream("/validation/rulesIGL.txt"));
  ...
  private List<String> allowedChangesUsage(String version, String from) throws IOException {
    ...
    HashMap<String, HashMap<String, List<String>>> hashMap =
        ((HashMap<String, HashMap<String, List<String>>>) o.get(version));
