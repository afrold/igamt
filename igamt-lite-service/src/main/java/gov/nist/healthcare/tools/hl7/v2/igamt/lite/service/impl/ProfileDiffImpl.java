...
public class ProfileDiffImpl {
	...
	public InputStream diffToPdf(Profile p1, Profile p2){
		try {
			...
			String stringDiff = (new ProfileDiffSerializationImpl(p1, p2, this)).serializeDiffToXML();
	...
	public InputStream diffToPdf2(Profile p1, Profile p2){
		...
		Font titleFont = FontFactory.getFont("/rendering/Arial Narrow.ttf",
				BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 13, Font.UNDERLINE | Font.BOLD | Font.NORMAL,
				BaseColor.BLACK);
		Font eltNameFont = FontFactory.getFont("/rendering/Arial Narrow.ttf",
				BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 13, Font.UNDERLINE | Font.NORMAL,
				BaseColor.BLACK);
		...
		try {
			...
			deltaDocument.add(new Paragraph("Changes in profile information", titleFont));
			...
			for (ElementChange ec :	this.profileInfoChanges){
				for (String field: ec.getChange().keySet()){
					...
					deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
					deltaDocument.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue") , valueFont));
					deltaDocument.add(new Paragraph("New value: " + ec.getChange().get(field).get("newvalue") , valueFont));
				}
			}
			...
			for (ElementChange ec :	this.messagesChanges){
				if (ec.getChangeType().equals("edit")){
					...
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(m.getDescription() + " edited", eltNameFont));
					for (String field: ec.getChange().keySet()){
						deltaDocument.add(Chunk.NEWLINE);
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
						deltaDocument.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue") , valueFont));
				...
				if (ec.getChangeType().equals("del")){
					deltaDocument.add(Chunk.NEWLINE);
					Message m = p1.getMessages().findOne(ec.getId());
					deltaDocument.add(new Paragraph(m.getId() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					Message m = p2.getMessages().findOne(ec.getId());
					deltaDocument.add(new Paragraph(m.getId() + " added", eltNameFont));				
				}
			}

			deltaDocument.add(Chunk.NEWLINE);
			deltaDocument.add(new Paragraph("Changes in segments and groups", titleFont));
			if (this.segrefOrGroupChanges.isEmpty())
				deltaDocument.add(new Paragraph("No changes", valueFont));
			for (ElementChange ec :	this.segrefOrGroupChanges){
				if (ec.getChangeType().equals("edit")){
					SegmentRefOrGroup srog = p1.getMessages().findOneSegmentRefOrGroup(ec.getId());
					deltaDocument.add(Chunk.NEWLINE);
					if (srog instanceof SegmentRef){
						Segment s = p1.getSegments().findOne(((SegmentRef) srog).getRef());
						deltaDocument.add(new Paragraph(s.getLabel() + " edited", eltNameFont));

					} else if (srog instanceof SegmentRef) {
						deltaDocument.add(new Paragraph(srog.getComment() + " edited", eltNameFont));						
					}
					for (String field: ec.getChange().keySet()){
						deltaDocument.add(Chunk.NEWLINE);
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
				...
				if (ec.getChangeType().equals("del")){
					deltaDocument.add(Chunk.NEWLINE);
					SegmentRefOrGroup srog = p1.getMessages().findOneSegmentRefOrGroup(ec.getId());
					deltaDocument.add(new Paragraph(srog.getId() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					SegmentRefOrGroup srog = p2.getMessages().findOneSegmentRefOrGroup(ec.getId());
					deltaDocument.add(new Paragraph(srog.getId() + " added", eltNameFont));
				}
			}
			...
			for (ElementChange ec :	this.segmentsChanges){
				...
				if (ec.getChangeType().equals("del")){
					deltaDocument.add(Chunk.NEWLINE);
					Segment s = p1.getSegments().findOne(ec.getId());
					deltaDocument.add(new Paragraph(s.getLabel() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					Segment s = p2.getSegments().findOne(ec.getId());
					deltaDocument.add(new Paragraph(s.getLabel() + " added", eltNameFont));
				}
			}
			...
			for (ElementChange ec :	this.fieldsChanges){
				if (ec.getChangeType().equals("edit")){
					...
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(f.getName() + " edited", eltNameFont));
					for (String field: ec.getChange().keySet()){
						deltaDocument.add(Chunk.NEWLINE);
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
						deltaDocument.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue") , valueFont));
				...
				if (ec.getChangeType().equals("del")){
					Field f = p1.getSegments().findOneField(ec.getId());
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(f.getName() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					Field f = p2.getSegments().findOneField(ec.getId());
					deltaDocument.add(new Paragraph(f.getName() + " added", eltNameFont));
				}
			}
			...
			for (ElementChange ec :	this.datatypesChanges){
				if (ec.getChangeType().equals("edit")){
					...
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(d.getLabel() + " edited", eltNameFont));
					for (String field: ec.getChange().keySet()){
						deltaDocument.add(Chunk.NEWLINE);
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
						deltaDocument.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue") , valueFont));
				...
				if (ec.getChangeType().equals("del")){
					Datatype d = p1.getDatatypes().findOne(ec.getId());
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(d.getLabel() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					Datatype d = p1.getDatatypes().findOne(ec.getId());
					deltaDocument.add(new Paragraph(d.getLabel() + " added", eltNameFont));
				}
			}

			deltaDocument.add(Chunk.NEWLINE);
			deltaDocument.add(new Paragraph("Changes in components", titleFont));
			...
			for (ElementChange ec :	this.componentsChanges){
				if (ec.getChangeType().equals("edit")){
					...
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(c.getName() + " edited", eltNameFont));
					for (String field: ec.getChange().keySet()){
						deltaDocument.add(Chunk.NEWLINE);
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
						deltaDocument.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue") , valueFont));
				...
				if (ec.getChangeType().equals("del")){
					Component c = p1.getDatatypes().findOneComponent(ec.getId());
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(c.getName() + " deleted", eltNameFont));
				}
				if (ec.getChangeType().equals("add")){
					Component c = p1.getDatatypes().findOneComponent(ec.getId());
					deltaDocument.add(Chunk.NEWLINE);
					deltaDocument.add(new Paragraph(c.getName()+ " added", eltNameFont));
				}
			}
			...
			deltaDocument.add(Chunk.NEWLINE);
			deltaDocument.add(new Paragraph("Changes in conformance statements", titleFont));
			...
			deltaDocument.add(Chunk.NEWLINE);
			deltaDocument.add(new Paragraph("Changes in predicates", titleFont));
	...
	private void addSectionToJson(JsonGenerator generator, String type, List<ElementChange> changes) throws IOException{
		generator.writeStartObject();
	...
	private void compareProfileInfo(Profile p1, Profile p2){
		...
		if (p1.getAccountId() != null & p2.getAccountId() != null)
			if (!(p1.getAccountId().equals(p2.getAccountId()))){
				ec.recordChange("accountId", p1.getAccountId().toString(), p2.getAccountId().toString());
			}
	...
	private void compareMetaData(ProfileMetaData metadata, ProfileMetaData metadata2){
		ElementChange ec = new ElementChange(metadata.getName(), "profileMetaData");
		...
		if (!metadata.getHl7Version().equals(metadata2.getHl7Version())){
			ec.recordChange("hl7Version", metadata.getHl7Version(), metadata2.getHl7Version());
		}
		if (!metadata.getSchemaVersion().equals(metadata2.getSchemaVersion())){
			ec.recordChange("version", metadata.getSchemaVersion(), metadata2.getSchemaVersion());
		}
		if (!metadata.getOrgName().equals(metadata2.getOrgName())){
			ec.recordChange("orgName", metadata.getOrgName(), metadata2.getOrgName());
		}
		if (!metadata.getStatus().equals(metadata2.getStatus())){
			ec.recordChange("status", metadata.getStatus(), metadata2.getStatus());
		}
		if (!metadata.getTopics().equals(metadata2.getTopics())){
			ec.recordChange("schemaTopics", metadata.getTopics(), metadata2.getTopics());
		}
		if (!metadata.getSubTitle().equals(metadata2.getSubTitle())){
			ec.recordChange("subtitle", metadata.getSubTitle(), metadata2.getSubTitle());
		}
		if (!metadata.getVersion().equals(metadata2.getVersion())){
			ec.recordChange("version", metadata.getVersion(), metadata2.getVersion());
		}
		...
		for (Message m : mlib1.getChildren() ){
			...
			if (mlib2.findOne(m.getId()) == null){
				...
				if ( !(m.getDescription().equals(m2.getDescription()))){
					ec.recordChange("Descriptiom", m.getDescription(), m2.getDescription());
				}
				...
				if ( !(m.getUsageNote().equals(m2.getUsageNote()))){
					ec.recordChange("UsageNote", m.getUsageNote(), m2.getUsageNote());
				}
				ec.setChangeType("edit");
			}
			if (ec.countChanges() != 0)
	...
	private void compareMessages(Messages mlib1, Messages mlib2, Segments slib1, Segments slib2){
		for (Message m: mlib1.getChildren()){
			for (SegmentRefOrGroup srog : m.getChildren()){
				...
				if (mlib2.findOneSegmentRefOrGroup(srog.getId()) == null){
					...
					SegmentRefOrGroup srog2 = mlib2.findOneSegmentRefOrGroup(srog.getId());
					if ( !(srog.getMin().equals(srog2.getMin()))){
						ec.recordChange("Min", srog.getMin().toString(), srog2.getMin().toString());
					}
					if ( !(srog.getMax().equals(srog2.getMax()))){
						ec.recordChange("Max", srog.getMax(), srog2.getMax());
					}
					if ( !(srog.getUsage().equals(srog2.getUsage()))){
						ec.recordChange("Usage", srog.getUsage().value(), srog.getUsage().value());
					}
					if ( !(srog.getComment().equals(srog2.getComment()))){
						ec.recordChange("Comment", srog.getComment(), srog2.getComment());
					}
					ec.setChangeType("edit");
				}
				if (ec.countChanges() != 0)
	...
	private void compareSegRef(SegmentRefOrGroup sr, Segments s1, Messages mlib2){
		...
		if (mlib2.findOneSegmentRefOrGroup(sr.getId()) == null){
			...
			if ( !(sr.getMin().equals(sr2.getMin()))){
				ec.recordChange("Min", sr.getMin().toString(), sr2.getMin().toString());
			}
			if ( !(sr.getMax().equals(sr2.getMax()))){
				ec.recordChange("Max", sr.getMax(), sr2.getMax());
			}
			if ( !(sr.getUsage().equals(sr2.getUsage()))){
				ec.recordChange("Usage", sr.getUsage().value(), sr.getUsage().value());
			}
	...
	private void compareGroups(SegmentRefOrGroup g, Segments slib1,Messages mlib2){
		...
		if (mlib2.findOneSegmentRefOrGroup(g.getId()) == null){
			...
			if ( !(g.getMin().equals(sr2.getMin()))){
				ec.recordChange("Min", g.getMin().toString(), sr2.getMin().toString());
			}
			if ( !(g.getMax().equals(sr2.getMax()))){
				ec.recordChange("Max", g.getMax().toString(), sr2.getMax().toString());
			}
			if ( !(g.getUsage().equals(sr2.getUsage()))){
				ec.recordChange("Usage", g.getUsage().value(), sr2.getUsage().value());
			}
		...
		for (Segment s : slib1.getChildren()){
			...
			if (slib2.findOne(s.getId()) == null){
				...
				if ( !(s.getDescription().equals(s2.getDescription())) ){
					ec.recordChange("Description", s.getDescription(), s2.getDescription());
				}
			...
			for (ConformanceStatement cs: s.getConformanceStatements()){
				...
				if (slib2.findOneConformanceStatement(cs.getId()) == null){
					...
					ConformanceStatement cs2 = slib2.findOneConformanceStatement(cs.getId());
					if (!cs.getAssertion().equals(cs2.getAssertion())){
						ec.recordChange("Assertion", cs.getAssertion(), cs2.getAssertion());
					}
					if (!cs.getConstraintTarget().equals(cs2.getConstraintTarget())){
						ec.recordChange("ConstraintTarget", cs.getConstraintTarget(), cs2.getConstraintTarget());
					}
					if (!cs.getDescription().equals(cs2.getDescription())){
						ec.recordChange("Description", cs.getDescription(), cs2.getDescription());
					}
					ec.setChangeType("edit");
				}
				if (ec.countChanges() != 0)
			...
			for (Predicate p : s.getPredicates()){
				...
				if (slib2.findOnePredicate(p.getId()) == null){
					...
					if (!p.getAssertion().equals(p2.getAssertion())){
						ec.recordChange("Assertion", p.getAssertion(), p2.getAssertion());
					}
					if (!p.getConstraintTarget().equals(p2.getConstraintTarget())){
						ec.recordChange("ConstraintTarget", p.getConstraintTarget(), p2.getConstraintTarget());
					}
					if (!p.getDescription().equals(p2.getDescription())){
						ec.recordChange("Description", p.getDescription(), p2.getDescription());
					}
					if (!p.getTrueUsage().equals(p2.getTrueUsage())){
						ec.recordChange("TrueUsage", p.getTrueUsage().value(), p2.getTrueUsage().value());
					}
					if (!p.getFalseUsage().equals(p2.getFalseUsage())){
						ec.recordChange("FalseUsage", p.getFalseUsage().value(), p2.getFalseUsage().value());
					}
					ec.setChangeType("edit");
				}
				if (ec.countChanges() != 0)
			...
			for (Field f : s.getFields()){
				...
				if (slib2.findOneField(f.getId()) == null){
					...
					if ( !f.getUsage().equals(f2.getUsage())){
						ec.recordChange("Usage", f.getUsage().value(), f2.getUsage().value());
					}
					if ( !f.getDatatype().equals(f2.getDatatype())){
						ec.recordChange("Datatype", f.getDatatype(), f2.getDatatype());
					}
					if ( !f.getMin().equals(f2.getMin())){
						ec.recordChange("Min", f.getMin().toString(), f2.getMin().toString());
					}
					if ( !f.getMinLength().equals(f2.getMinLength())){
						ec.recordChange("MinLength", f.getMinLength().toString(), f2.getMinLength().toString());
					}
					if ( !f.getMax().equals(f2.getMax())){
						ec.recordChange("Max", f.getMax(), f2.getMax());
					}
					if ( !f.getMaxLength().equals(f2.getMaxLength())){
						ec.recordChange("MaxLength", f.getMaxLength(), f2.getMaxLength());
					}
					if ( !f.getText().equals(f2.getText())){
						ec.recordChange("Text", f.getText(), f2.getText());
					}
					if ( !f.getComment().equals(f2.getComment())){
						ec.recordChange("Comment", f.getComment(), f2.getComment());
					}
					if ( !f.getConfLength().equals(f2.getConfLength())){
						ec.recordChange("ConfLength", f.getConfLength(), f2.getConfLength());
					}
					if (f.getTable() != null & f2.getTable() != null){
						if ( !f.getTable().equals(f2.getTable())){
							ec.recordChange("Table", f.getTable(), f2.getTable());
						}
					}
					if ( f.getBindingLocation() != null & f2.getBindingLocation() != null){
						if ( !f.getBindingLocation().equals(f2.getBindingLocation())){
							ec.recordChange("BindingLocation", f.getBindingLocation(), f2.getBindingLocation());
						}
					}
					if ( f.getBindingStrength() != null & f2.getBindingStrength() != null){
						if ( !f.getBindingStrength().equals(f2.getBindingStrength())){
							ec.recordChange("BindingStrength", f.getBindingStrength(), f.getBindingStrength());
						}
					}
					ec.setChangeType("edit");
				}
		...
		for (Datatype dt : dlib1.getChildren()){
			...
			if (dlib2.findOne(dt.getId()) == null){
				...
				if (!(dt.getDescription().equals(dt2.getDescription()))){
					ec.recordChange("Description", dt.getDescription(), dt2.getDescription());
				}
				if (!(dt.getComment().equals(dt2.getComment()))){
					ec.recordChange("Comment", dt.getComment(), dt2.getComment());
				}
				if (!(dt.getUsageNote().equals(dt2.getUsageNote()))){
					ec.recordChange("UsageNote", dt.getUsageNote(), dt2.getUsageNote());
				}
				ec.setChangeType("edit");
			}
			if (ec.countChanges() != 0)
			...
			for (ConformanceStatement cs: dt.getConformanceStatements()){
				...
				if (dlib2.findOneConformanceStatement(cs.getId()) == null){
					...
					ConformanceStatement cs2 = dlib2.findOneConformanceStatement(cs.getId());
					if (!cs.getAssertion().equals(cs2.getAssertion())){
						ec.recordChange("Assertion", cs.getAssertion(), cs2.getAssertion());
					}
					if (!cs.getConstraintTarget().equals(cs2.getConstraintTarget())){
			...
			for (Component c : dt.getComponents()){
				...
				if (dlib2.findOneComponent(c.getId()) == null){
					...
					if ( !(c.getDatatype().equals(c2.getDatatype()))){
						ec.recordChange("Datatype", c.getDatatype(), c2.getDatatype());
					}
					if ( !(c.getName().equals(c2.getName()))){
						ec.recordChange("Name", c.getName(), c2.getName());
					}
					if ( !(c.getUsage().equals(c2.getUsage()))){
						ec.recordChange("Usage", c.getUsage().value(), c2.getUsage().value());
					}
					if ( !(c.getConfLength().equals(c2.getConfLength()))){
						ec.recordChange("ConfLength", c.getConfLength(), c2.getConfLength());
					}
					if ( !(c.getMinLength().equals(c2.getMinLength()))){
						ec.recordChange("MinLength", c.getMinLength().toString(), c2.getMinLength().toString());
					}
					if ( !(c.getMaxLength().equals(c2.getMaxLength()))){
						ec.recordChange("MaxLength", c.getMaxLength(), c2.getMaxLength());
					}
					if ( !(c.getBindingLocation().equals(c2.getBindingLocation()))){
						ec.recordChange("BindingLocation", c.getBindingLocation(), c2.getBindingLocation());
					}
					if ( !(c.getBindingStrength().equals(c2.getBindingStrength()))){
						ec.recordChange("BindingStrength", c.getBindingStrength(), c2.getBindingStrength());
					}
					if ( c.getTable() != null & c2.getTable() != null){
						if ( !(c.getTable().equals(c2.getTable()))){
							ec.recordChange("Table", c.getTable(), c2.getTable());
						}
					}
					if ( !(c.getComment().equals(c2.getComment()))){
						ec.recordChange("Comment", c.getComment(), c2.getComment());
					}
			...
			for (Code c : t.getCodes()){
				...
				if (tables2.findOneCode(c.getId()) == null){
					...
					if ( !(c.getLabel().equals(c2.getLabel()))){
						ec.recordChange("DisplayName", c.getLabel(), c2.getLabel());
					}
					if ( !(c.getCodesys().equals(c2.getCodesys()))){
						ec.recordChange("CodeSys", c.getCodesys(), c2.getCodesys());
					}
	...
	public void compare(Profile p1, Profile p2) {
		...
		this.compareMessages(p1.getMessages(), p2.getMessages(), p1.getSegments(), p2.getSegments());
		this.compareSegments(p1.getSegments(), p2.getSegments()); //includes constraints
		this.compareFields(p1.getSegments(), p2.getSegments());
		this.compareDatatypes(p1.getDatatypes(), p2.getDatatypes()); //includes constraints
	...
	public List<ElementChange> findFieldsByParentIdAndChangeType(String parentId, String changeType){
		List<ElementChange> rst = new ArrayList<ElementChange>();
		List<String> symbols = Arrays.asList(new String[] {"add","del","edit","*"});
		if (symbols.contains(changeType)){
			if (this.fieldsChanges != null){
				if (changeType.equals("*")){
					...
					for (ElementChange s : this.fieldsChanges) {
						if (s.getParent().equals(parentId) & s.getChangeType().equals(changeType)) {
							rst.add(s);
						}
	...
	public List<ElementChange> findComponentsByParentIdAndChangeType(String parentId, String changeType){
		List<ElementChange> rst = new ArrayList<ElementChange>();
		List<String> symbols = Arrays.asList(new String[] {"add","del","edit","*"});
		if (symbols.contains(changeType)){
			if (this.componentsChanges != null){
	...
	public List<ElementChange> findCodesByParentIdAndChangeType(String parentId, String changeType){
		List<ElementChange> rst = new ArrayList<ElementChange>();
		List<String> symbols = Arrays.asList(new String[] {"add","del","edit","*"});
		if (symbols.contains(changeType)){
			if (this.codesChanges != null){
	...
	public List<ElementChange> findAddedFieldsByParentId(String parentId){
		...
		if (this.fieldsChanges != null)
			for (ElementChange s : this.fieldsChanges) {
				if (s.getParent().equals(parentId) & s.getChangeType().equals("add")) {
					rst.add(s);
				}
			}
		return rst;
	...
	public List<ElementChange> findDeletedFieldsByParentId(String parentId){
		...
		if (this.fieldsChanges != null)
			for (ElementChange s : this.fieldsChanges) {
				if (s.getParent().equals(parentId) & s.getChangeType().equals("del")) {
					rst.add(s);
				}
			}
		return rst;
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			File tmpFile = new File("/Users/marieros/Documents/testXslt/nvo/delta.pdf");
