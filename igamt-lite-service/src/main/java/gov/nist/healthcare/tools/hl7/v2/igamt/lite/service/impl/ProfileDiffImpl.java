...
public class ProfileDiffImpl {
  ...
  public InputStream diffToPdf(Profile p1, Profile p2) {
    try {
      ...
      nu.xom.Document stylesheet =
          builder.build(this.getClass().getResourceAsStream("/rendering/profile2b.xsl"));
  ...
  public InputStream diffToPdf2(Profile p1, Profile p2) {
    ...
    Font titleFont =
        FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,
            13, Font.UNDERLINE | Font.BOLD | Font.NORMAL, BaseColor.BLACK);
    Font eltNameFont =
        FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,
            13, Font.UNDERLINE | Font.NORMAL, BaseColor.BLACK);
    Font fieldFont =
        FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,
            11, Font.ITALIC, BaseColor.BLACK);
    Font valueFont =
        FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,
      ...
      for (ElementChange ec : this.profileInfoChanges) {
        for (String field : ec.getChange().keySet()) {
          ...
          deltaDocument.add(new Paragraph("Base value: "
              + ec.getChange().get(field).get("basevalue"), valueFont));
          deltaDocument.add(new Paragraph(
              "New value: " + ec.getChange().get(field).get("newvalue"), valueFont));
        }
      }
        ...
        if (ec.getChangeType().equals("edit")) {
          ...
          for (String field : ec.getChange().keySet()) {
            ...
            deltaDocument.add(new Paragraph("New value: "
                + ec.getChange().get(field).get("newvalue"), valueFont));
          }
        }
  ...
  private void compareMessages(Messages mlib1, Messages mlib2, SegmentLibrary slib1,
      SegmentLibrary slib2) {
    // for (Message m : mlib1.getChildren()) {
  ...
  public void compare(Profile p1, Profile p2) {
    ...
    this.compareMessages(p1.getMessages(), p2.getMessages(), p1.getSegmentLibrary(),
        p2.getSegmentLibrary());
  ...
  public List<ElementChange> findComponentsByParentIdAndChangeType(String parentId,
      String changeType) {
    List<ElementChange> rst = new ArrayList<ElementChange>();
    List<String> symbols = Arrays.asList(new String[] {"add", "del", "edit", "*"});
    if (symbols.contains(changeType)) {
