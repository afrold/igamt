...
public class ProfileDiffImpl {
	...
	public InputStream diffToPdf(Profile p1, Profile p2) {
		try {
			...
			FileUtils.writeStringToFile(tmpXmlFile, stringDiff, Charset.forName("UTF-8"));

			// Apply stylesheet
			File tmpHtmlFile = File.createTempFile("DiffTemp", ".html");
			Builder builder = new Builder();
			nu.xom.Document input = builder.build(tmpXmlFile);
			nu.xom.Document stylesheet = builder.build(this.getClass().getResourceAsStream("/rendering/profile2b.xsl"));
	...
	public InputStream diffToPdf2(Profile p1, Profile p2) {
		...
		Font titleFont = FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 13,
				Font.UNDERLINE | Font.BOLD | Font.NORMAL, BaseColor.BLACK);
		Font eltNameFont = FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED,
				13, Font.UNDERLINE | Font.NORMAL, BaseColor.BLACK);
		Font fieldFont = FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 11,
				Font.ITALIC, BaseColor.BLACK);
		Font valueFont = FontFactory.getFont("/rendering/Arial Narrow.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 11,
		...
		try {
			...
			for (ElementChange ec : this.profileInfoChanges) {
				for (String field : ec.getChange().keySet()) {
					...
					deltaDocument
							.add(new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue"), valueFont));
					deltaDocument
							.add(new Paragraph("New value: " + ec.getChange().get(field).get("newvalue"), valueFont));
				}
			}
			...
			for (ElementChange ec : this.messagesChanges) {
				if (ec.getChangeType().equals("edit")) {
					...
					for (String field : ec.getChange().keySet()) {
						...
						deltaDocument.add(new Paragraph("Field: " + field, fieldFont));
						deltaDocument.add(
								new Paragraph("Base value: " + ec.getChange().get(field).get("basevalue"), valueFont));
						deltaDocument.add(
								new Paragraph("New value: " + ec.getChange().get(field).get("newvalue"), valueFont));
					}
				}
			...
			for (ElementChange ec : this.segmentsChanges) {
				// if (ec.getChangeType().equals("edit")) {
				// Segment s =
				// p1.getSegmentLibrary().findOneSegmentById(ec.getId());
				// deltaDocument.add(Chunk.NEWLINE);
				...
				// if (ec.getChangeType().equals("add")) {
				// Segment s =
				// p2.getSegmentLibrary().findOneSegmentById(ec.getId());
	...
	private void addSectionToJson(JsonGenerator generator, String type, List<ElementChange> changes)
			throws IOException {
		generator.writeStartObject();
	...
	private void compareMessages(Messages mlib1, Messages mlib2, SegmentLibrary slib1, SegmentLibrary slib2) {
		// for (Message m : mlib1.getChildren()) {
	...
	private void compareGroups(SegmentRefOrGroup g, SegmentLibrary slib1, Messages mlib2) {
	...
	public void compare(Profile p1, Profile p2) {
		...
		this.compareMessages(p1.getMessages(), p2.getMessages(), p1.getSegmentLibrary(), p2.getSegmentLibrary());
	...
	public static void main(String[] args) throws IOException {
		// try {
		// IGDocumentSerialization4ExportImpl test1 = new
		// IGDocumentSerialization4ExportImpl();
		//
		// Profile p1 = test1
		...
		// Segment segment =
		// p2.getSegmentLibrary().findOneSegmentById(segmentRef.getRef());
		...
		// Segment segment3 =
		// p1.getSegmentLibrary().findOneSegmentById(segmentRef3.getRef());
		...
		// segment.setText1("<h2>Test format!</h2><p>textAngular WYSIWYG Text
		// Editor</p><p><b>Features:</b></p><ol><li>Two-Way-Binding</li><li
		// style=\"color: ;\"><b>Theming</b> Options</li><li>Simple Editor
		// Instance Creation</li></ol><p><b>Link test:</b> <a
		// href=\"https://github.com/fraywing/textAngular\">Here</a> </p>");
		//
