...
public class ProfileCreationImpl implements ProfileCreationService {
  ...
  public List<String[]> summary(String hl7Version, List<String> messageIds) {
    ...
    List<Profile> pl =
        profileRepository
            .findByScopeAndMetaData_Hl7Version(IGDocumentScope.HL7STANDARD, hl7Version);
    for (Profile p : pl) {
      for (Message m : p.getMessages().getChildren()) {
        if (!messageIds.contains(m.getId())) {
          String[] msgDesc =
              new String[] {m.getId(), m.getEvent(), m.getStructID(), m.getDescription()};
          rst.add(msgDesc);
        }
      }
    }
  ...
  @Override
  public Profile createIntegratedProfile(List<String> msgIds, String hl7Version, Long accountId)
  ...
  @Override
  public Profile updateIntegratedProfile(List<String> msgIds, Profile pTarget)
      throws ProfileException {
    // Update profile with additional messages.
    // String hl7Version = pTarget.getMetaData().getHl7Version();
    // Profile pSource =
    // profileRepository.findByScopeAndMetaData_Hl7Version(IGDocumentScope.HL7STANDARD, hl7Version)
    // .get(0);
    // addMessages(msgIds, pSource, pTarget);
    return pTarget;
  }
