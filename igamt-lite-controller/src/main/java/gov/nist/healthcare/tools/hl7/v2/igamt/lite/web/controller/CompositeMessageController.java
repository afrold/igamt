...
public class CompositeMessageController extends CommonController {
  ...
  @RequestMapping(value = "/getsegorgrp", method = RequestMethod.POST,
      produces = "application/json")
  public List<SegmentOrGroup> getSegOrGrps(
      @RequestBody List<SegmentOrGroupLink> segmentOrGroupLinks) {
  ...
  public CompositeMessage save(@PathVariable("igId") String igId,
      @RequestBody CompositeMessage compositeMessage) throws IGDocumentException {
  ...
  public void deleteCompositeMessageById(@PathVariable("id") String id) {
    CompositeMessage cm = compositeMessageService.findById(id);
    for (SegmentOrGroupLink segOrGrpLink : cm.getChildren()) {
      compositeMessageService.deleteSegOrGrp(segOrGrpLink.getId());
    }
    compositeMessageService.delete(id);
  ...
  public List<SegmentOrGroup> save(@RequestBody List<SegmentOrGroup> segOrGrps)
