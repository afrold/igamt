...
public class WebBeanConfig {
  ...
  IGDocumentConfiguration igDocumentConfig() {
    ...
    config.setDomainVersions(toSet(new String[] {"2.0", "2.1", "2.2", "2.3", "2.3.1", "2.4", "2.5",
        "2.5.1", "2.6", "2.7", "2.8"}));
    ...
    config.setConstraintTypes(toSet(new String[] {"valued", "a literal value",
        "one of list values", "one of codes in ValueSet", "formatted value",
        "identical to another node", "equal to another node", "not-equal to another node",
        "greater than another node", "equal to or greater than another node",
        "less than another node", "equal to or less than another node", "equal to", "not-equal to",
        "greater than", "equal to or greater than", "less than", "equal to or less than",
        "valued sequentially starting with the value '1'"}));
    config.setPredefinedFormats(toSet(new String[] {"ISO-compliant OID", "Alphanumeric", "YYYY",
        "YYYYMM", "YYYYMMDD", "YYYYMMDDhh", "YYYYMMDDhhmm", "YYYYMMDDhhmmss", "YYYYMMDDhhmmss.sss",
        "YYYY+-ZZZZ", "YYYYMM+-ZZZZ", "YYYYMMDD+-ZZZZ", "YYYYMMDDhh+-ZZZZ", "YYYYMMDDhhmm+-ZZZZ",
  ...
  ProfileConfiguration profileConfig() {
    ...
    config.setDomainVersions(toSet(new String[] {"2.0", "2.1", "2.2", "2.3", "2.3.1", "2.4", "2.5",
        "2.5.1", "2.6", "2.7", "2.8"}));
    config.setSchemaVersions(toSet(new String[] {"1.0", "1.5", "2.0", "2.5"}));
    config.setUsages(toSet(new String[] {"R", "RE", "O", "C", "X", "B", "W"}));
    config.setCodeUsages(toSet(new String[] {"R", "P", "E"}));
    config.setCodeSources(toSet(new String[] {"HL7", "Local", "Redefined", "SDO"}));
    config.setTableStabilities(toSet(new String[] {"Static", "Dynamic"}));
    config.setTableContentDefinitions(toSet(new String[] {"Extensional", "Intensional"}));
