...
public class SegmentLibraryController extends CommonController {
	...
	public List<SegmentLibrary> findByScopes(@RequestBody List<String> scopes) {
		...
		List<SegmentLibrary> segmentLibraries = segmentLibraryService
	...
	public List<Segment> findByScopesAndVersion(
		...
		log.info("Fetching the segment library. scope="
				+ scopesAndVersion.getScopes() + " hl7Version="
				+ scopesAndVersion.getHl7Version());
		List<Segment> segments = null;
		try {
			segments = segmentService.findByScopesAndVersion(
					scopesAndVersion.getScopes(),
					scopesAndVersion.getHl7Version());
			if (segments == null) {
				throw new NotFoundException(
						"Segment not found for scopesAndVersion="
								+ scopesAndVersion);
			}
		} catch (Exception e) {
			log.error("", e);
	...
	public List<SegmentLibrary> findByAccountId(
		...
		List<SegmentLibrary> result = segmentLibraryService.findByAccountId(
	...
	public SegmentLibrary create(@RequestBody LibraryCreateWrapper dtlcw) {
		...
		return segmentLibraryService.create(dtlcw.getName(), dtlcw.getExt(),
	...
	public LibrarySaveResponse save(@RequestBody SegmentLibrary segmentLibrary)
		...
		log.info("Saving the " + segmentLibrary.getScope()
				+ " segment library.");
	...
	public ResponseMessage addSegment(
			@PathVariable("segLibId") String segLibId,
			@RequestParam("segId") String segId) throws SegmentSaveException,
	...
	public List<SegmentLibrary> findLibrariesByFlavorName(
		...
		Account account = accountRepository.findByTheAccountsUsername(u
				.getUsername());
		List<SegmentLibrary> libraries = segmentLibraryService
